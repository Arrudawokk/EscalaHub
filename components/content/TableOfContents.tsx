    ...
    <nav aria-label="Sumário do artigo" className="surface-panel p-5">
      ...
      <ol className="mt-4 space-y-1">
        {sections.map((section, index) => (
          <li key={section.id}>
            <a href={`#${section.id}`} className="flex min-h-10 items-start gap-3 rounded-lg px-2 py-2 text-sm leading-5 text-zinc-400 outline-none transition-colors hover:bg-white/[.04] hover:text-white focus-visible:ring-2 focus-visible:ring-[#3B82F6]">
