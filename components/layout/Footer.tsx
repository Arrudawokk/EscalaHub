      ...
      <div className="container-default">
        <div className="grid gap-12 border-b border-white/[.07] pb-14 md:grid-cols-[1.2fr_.8fr_.8fr_.8fr]">
          <div className="max-w-sm">
            <Link href="/" aria-label="EscalaHub — início" className="inline-flex min-h-11 items-center rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-[#3B82F6]"><BrandLogo /></Link>
            ...
            <a href={`mailto:${siteConfig.contactEmail}`} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-lg py-2 text-sm font-semibold text-zinc-300 outline-none hover:text-white focus-visible:ring-2 focus-visible:ring-[#3B82F6]"><FiMail aria-hidden="true" /> {siteConfig.contactEmail}</a>
          </div>
          {footerGroups.map((group) => (
            <nav key={group.title} aria-label={group.title}>
              <p className="text-[10px] font-black uppercase tracking-[.15em] text-zinc-500">{group.title}</p>
              <ul className="mt-4 space-y-1">{group.links.map((link) => <li key={link.href}><Link href={link.href} className="inline-flex min-h-10 items-center rounded px-1 text-sm text-zinc-400 outline-none transition hover:text-white focus-visible:ring-2 focus-visible:ring-[#3B82F6]">{link.label}</Link></li>)}</ul>
            </nav>
          ))}
        </div>
