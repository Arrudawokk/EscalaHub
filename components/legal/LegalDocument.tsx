        ...
        <div className="container-default grid max-w-5xl gap-10 py-14 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-20">
          <aside className="lg:sticky lg:top-24 lg:h-fit">
            <nav aria-label="Sumário" className="rounded-[20px] border border-white/[.08] bg-white/[.025] p-4">
              ...
              <ul className="mt-3 space-y-1">{sections.map((section, index) => <li key={section.title}><a href={`#secao-${index + 1}`} className="block rounded-lg px-2 py-2 text-sm leading-5 text-zinc-400 outline-none transition-colors hover:bg-white/[.04] hover:text-white focus-visible:ring-2 focus-visible:ring-blue-400">{section.title}</a></li>)}</ul>
            </nav>
          </aside>
