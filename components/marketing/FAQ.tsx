...
export function FAQ({ product }: { product: Product }) {
  const questions = [
    ["Para quem é este guia?", "Para iniciantes, gestores e empreendedores que querem estruturar campanhas com método e tomar decisões baseadas em dados."],
    ["Como recebo o acesso?", "O acesso é liberado por e-mail logo após a confirmação do pagamento."],
    ["Preciso ter experiência com anúncios?", "Não. O conteúdo começa pelos fundamentos e avança até otimização e escala."],
    ["Existe mensalidade?", `Não. O pagamento de ${formatProductPrice(product)} é único e o acesso ao material é ${product.accessLabel.toLowerCase()}.`],
    ["Como funciona a garantia?", `Você tem ${product.guaranteeDays} dias para acessar, avaliar o conteúdo e solicitar reembolso dentro desse prazo.`],
  ];
  return (
